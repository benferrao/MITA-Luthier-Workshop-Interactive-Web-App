  ...
  <div class="home">
    <section class="hero">
      ...
      <div class="violin-container" ref="containerEl">
        ...
        <!-- IN-PAGE HINT — shown after cards close, inside violin area -->
        <transition name="hint-fade">
          <div v-if="guide.hintActive" class="violin-hint-box">
            <div v-if="guide.hint===0" class="hint-inner">
              <span class="hint-icon">↔</span>
              <strong>Drag to rotate</strong>
              <p>Spin the violin to see it from all angles</p>
              <div class="hint-row">
                <button class="hint-btn" @click="guide.hint=1">Got it →</button>
                <button class="hint-skip" @click="skipAll">Skip</button>
              </div>
            </div>
            <div v-if="guide.hint===1" class="hint-inner">
              <span class="hint-icon">✦</span>
              <strong>Click the glowing dots</strong>
              <p>Each dot reveals a part of the violin — try clicking one!</p>
              <div class="hint-row">
                <button class="hint-btn" @click="guide.hint=2">I found one! →</button>
                <button class="hint-skip" @click="skipAll">Skip</button>
              </div>
            </div>
            <div v-if="guide.hint===2" class="hint-inner hint-final">
              <span class="hint-icon">🔍</span>
              <strong>Ready to explore more?</strong>
              <p>Use the tools below to go deeper into the workshop</p>
              <div class="hint-final-btns">
                <NuxtLink to="/" class="hint-cta-btn" @click="skipAll">Explore Tools →</NuxtLink>
                <NuxtLink to="/tour" class="hint-cta-ghost" @click="skipAll">Guided Tour</NuxtLink>
              </div>
              <button class="hint-skip" @click="skipAll">Close guide</button>
            </div>
          </div>
        </transition>
      </div>
    ...
    <!-- ══════════════════════════════════
         ONBOARDING CARDS (2 steps only)
         Style matches the rest of the page
         ══════════════════════════════════ -->
    <transition name="guide-fade">
      <div v-if="guide.cardActive" class="guide-overlay">
        <div class="guide-card">
          <div class="guide-dots">
            <span class="guide-dot" :class="{active: guide.step===0}"></span>
            <span class="guide-dot" :class="{active: guide.step===1}"></span>
          </div>
          <!-- Step 0 -->
          <div v-if="guide.step===0" class="guide-step">
            <div class="guide-emoji">🎻</div>
            <h2>Welcome to<br><em>The Violin Workshop</em></h2>
            <p>Have you ever wondered how a violin is made? Let us guide you through Cremona's ancient luthier tradition.</p>
            <div class="guide-btns">
              <button class="gbtn-primary" @click="guide.step=1">Yes, show me! →</button>
              <button class="gbtn-ghost" @click="skipAll">Skip intro</button>
            </div>
          </div>
          <!-- Step 1 -->
          <div v-if="guide.step===1" class="guide-step">
            <div class="guide-emoji">🏙️</div>
            <h2>Do you know<br><em>Cremona?</em></h2>
            <p>A small city in northern Italy — birthplace of the finest violins ever made. Stradivari, Guarneri and Amati all worked here for centuries.</p>
            <div class="guide-btns">
              <button class="gbtn-primary" @click="startViolinGuide">Explore the violin →</button>
              <button class="gbtn-ghost" @click="guide.step=0">← Back</button>
            </div>
          </div>
        </div>
      </div>
...
function startViolinGuide() {
  guide.cardActive = false
  guide.hintActive = true
  guide.hint = 0
}
function skipAll() {
  guide.cardActive = false
  guide.hintActive = false
}

// ── ALL ORIGINAL CODE BELOW — UNCHANGED ──
const tasks = [
...
const hotspots = reactive([
  {
    ...
    task: 'Task 1 · Point & Ask', taskColor: '#C8860A', link: '/',
  },
  {
    ...
    task: 'Task 2 · Identify Tools', taskColor: '#6aaee0', link: '/',
  },
  {
    key:'ribs', label:'Ribs',
    ...
    task: 'Task 3 · Guided Tour', taskColor: '#6ed98a', link: '/tour',
  },
  {
    key:'lower-bout', label:'Lower Bout',
...
function onHotspot(h) {
  ...
  if (guide.hintActive && guide.hint === 1) guide.hint = 2
}

function imgError(e) { e.target.style.display = 'none' }
function goTo(path) { navigateTo(path) }
...
function initThree() {
  ...
  s.onload = () => {
    new T.GLTFLoader().load('/violin.glb', gltf => {
      ...
      model.traverse(ch => {
        if (ch.name && (ch.name.startsWith('\u7403\u4f53') || ch.name === '\u878d\u7403')) ch.visible = false
        if (ch.name && ch.name.includes('bow')) ch.visible = false
      })
  ...
  canvas.addEventListener('click', e => {
    ...
    if (hits.length) onHotspot(hotspots[hits[0].object.userData.index])
  })

  const el = container
  function onStart(cx) { dragging=true; lastX=cx; velY=0; autoTimer=0; el.style.cursor='grabbing' }
  function onMove(cx) { if(!dragging)return; const dx=cx-lastX; velY=dx*0.013; tRotY+=dx*0.013; lastX=cx }
  ...
  canvas.addEventListener('mousemove', e => {
    if(dragging)return; ndcFromEvent(e); ray.setFromCamera(m2,cam)
    el.style.cursor = ray.intersectObjects(hSpheres.map(h=>h.sp)).length ? 'pointer' : 'grab'
  })
  ...
  function tick() {
    ...
    if (!dragging) { autoTimer+=dt; velY*=0.94; tRotY+=velY; if(autoTimer>3)tRotY+=0.003 }
    rotY += (tRotY-rotY)*0.1
    ...
    hSpheres.forEach((h,i) => {
      ...
      hotspots[i].visible = proj.z<0.999 && proj.z>-1
    })
...
onMounted(() => {
  ...
  if(st){new IntersectionObserver(([e])=>{if(e.isIntersecting){animCount(sn1.value,16,'',1400);animCount(sn2.value,8,'',1400);animCount(sn3.value,200,'h',1400)}},{threshold:0.3}).observe(st)}
})
onBeforeUnmount(()=>{
...
.hero{height:100vh;background:linear-gradient(160deg,#2c1810 0%,#8B4513 55%,#c8860a 100%);display:grid;grid-template-columns:1fr 1fr;overflow:hidden}
.hero-text{grid-column:1;grid-row:1;display:flex;flex-direction:column;justify-content:center;padding:60px 40px 60px 64px;z-index:10}
...
.violin-container{grid-column:2;grid-row:1;position:relative;cursor:grab;user-select:none;height:100%;overflow:hidden}
.violin-container:active{cursor:grabbing}
.three-canvas{position:absolute;inset:0;width:100%;height:100%;display:block}
.hotspot{position:absolute;width:26px;height:26px;transform:translate(-50%,-50%);background:none;border:none;padding:0;cursor:pointer;z-index:20;display:flex;align-items:center;justify-content:center;transition:opacity .3s ease}
.hs-ring{position:absolute;inset:-5px;border:2px solid rgba(255,255,255,.7);border-radius:50%;animation:hsp 2s ease-in-out var(--delay,0s) infinite}
.hs-dot{width:14px;height:14px;background:white;border-radius:50%;box-shadow:0 0 18px rgba(255,255,255,1),0 0 8px rgba(200,134,10,.9);z-index:1;transition:transform .2s;flex-shrink:0}
.hotspot:hover .hs-dot{transform:scale(1.4)}
.hotspot:hover .hs-tip{opacity:1;transform:translateX(-50%) translateY(0)}
.hs-tip{position:absolute;bottom:calc(100%+12px);left:50%;transform:translateX(-50%) translateY(8px);white-space:nowrap;background:rgba(44,24,16,.95);color:#f5d090;font-family:'Inter',sans-serif;font-size:11px;letter-spacing:.1em;text-transform:uppercase;padding:5px 14px;border-radius:20px;pointer-events:none;opacity:0;transition:all .2s;border:1px solid rgba(200,134,10,.4)}
...
.side-panel{position:fixed;right:0;top:0;bottom:0;width:380px;background:white;border-left:1px solid #ede5d8;box-shadow:-8px 0 32px rgba(0,0,0,.18);display:flex;flex-direction:column;transform:translateX(100%);transition:transform .4s cubic-bezier(.16,1,.3,1);z-index:300;overflow-y:auto}
...
.panel-img-label{position:absolute;bottom:14px;left:18px;font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:rgba(245,239,228,.8);background:rgba(44,24,16,.5);padding:4px 10px;border-radius:12px;border:1px solid rgba(245,239,228,.2)}
.pbody{padding:24px}
...
.hint-inner strong{
  ...
  color:#f5d090;margin-bottom:6px;
}
.hint-inner p{font-size:13px;color:rgba(255,255,255,.75);line-height:1.5;margin-bottom:14px}
.hint-row{display:flex;align-items:center;gap:12px}
.hint-btn{
  background:white;color:#2c1810;border:none;
  border-radius:20px;padding:9px 20px;
  font-size:13px;font-weight:600;cursor:pointer;
  font-family:'Inter',sans-serif;transition:all .2s;
}
.hint-btn:hover{background:#f5ede0;transform:translateY(-1px)}
.hint-skip{
...
.hint-skip:hover{color:rgba(255,255,255,.7)}
.hint-final{text-align:center}
.hint-final-btns{display:flex;flex-direction:column;gap:8px;margin-bottom:12px}
.hint-cta-btn{
  display:block;padding:11px 20px;
  background:white;color:#2c1810;border:none;border-radius:20px;
  font-size:13px;font-weight:600;cursor:pointer;
  text-decoration:none;font-family:'Inter',sans-serif;
  transition:all .2s;text-align:center;
}
.hint-cta-btn:hover{background:#f5ede0;transform:translateY(-1px)}
.hint-cta-ghost{
  display:block;padding:11px 20px;
...
.hint-cta-ghost:hover{background:rgba(255,255,255,.2)}
...
.guide-dots{display:flex;justify-content:center;gap:8px;margin-bottom:32px}
.guide-dot{width:8px;height:8px;border-radius:50%;background:#ede5d8;transition:background .3s}
.guide-dot.active{background:#8B4513}
.guide-step{text-align:center}
.guide-emoji{font-size:56px;margin-bottom:20px}
.guide-step h2{
  font-family:'Playfair Display',serif;
  font-size:30px;font-weight:700;
  color:#2c1810;line-height:1.2;margin-bottom:14px;
}
.guide-step h2 em{font-style:italic;color:#8B4513;font-weight:400}
.guide-step p{font-size:15px;color:#666;line-height:1.75;margin-bottom:30px}
.guide-btns{display:flex;flex-direction:column;gap:10px}
.gbtn-primary{
  padding:14px 28px;background:#8B4513;color:white;
...
.gbtn-primary:hover{background:#6d3410;transform:translateY(-2px)}
.gbtn-ghost{
  padding:14px 28px;background:transparent;color:#8B4513;
  border:2px solid #ede5d8;border-radius:30px;font-size:14px;font-weight:500;
  cursor:pointer;transition:all .22s;font-family:'Inter',sans-serif;
  text-decoration:none;display:block;text-align:center;
}
.gbtn-ghost:hover{border-color:#8B4513;background:#fdf8f4}

/* Transitions */
.guide-fade-enter-active,.guide-fade-leave-active{transition:opacity .35s,transform .35s}
.guide-fade-enter-from,.guide-fade-leave-to{opacity:0;transform:scale(.96)}
.hint-fade-enter-active,.hint-fade-leave-active{transition:opacity .3s,transform .3s}
